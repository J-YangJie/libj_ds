...
static void demo_about_erase(void)
{
    ...
    ret = cds->remove(demo, _tok(id[0]));                     // [ '123', '?混搭33*&', 'test' ], return 1(1 element has been removed)

    ret = cds->remove_if(demo, demo_remove_if_condition_k);   // [ 'test' ], return 2(2 element has been removed)
